	...
	var runInContext = function runInContext(context) {
		...
		function baseWrapperValue(value, actions) {
			...
			return arrayReduce(
				actions,
				(result, action) =>
					action.func.apply(action.thisArg, arrayPush([result], action.args)),
				result,
			);
		}
		 ...
		 */
		var clearTimeout = ctxClearTimeout || ((id) => root.clearTimeout(id));
		...
		function createCompounder(callback) {
			return (string) =>
				arrayReduce(words(deburr(string).replace(reApos, "")), callback, "");
		}
		...
		function createInverter(setter, toIteratee) {
			return (object, iteratee) =>
				baseInverter(object, setter, toIteratee(iteratee), {});
		}
		 ...
		 */
		var setTimeout =
			ctxSetTimeout || ((func, wait) => root.setTimeout(func, wait));
		...
		var pullAt = flatRest((array, indexes) => {
			...
			basePullAt(
				array,
				arrayMap(indexes, (index) =>
					isIndex(index, length) ? +index : index,
		...
		var wrapperAt = flatRest(function (paths) {
			var length = paths.length,
				...
				interceptor = (object) => baseAt(object, paths);
		 ...
		 */
		var delay = baseRest((func, wait, args) =>
			baseDelay(func, toNumber(wait) || 0, args),
		);
		 ...
		 * // => ['a', 'b', 'c']
		 */
		var rearg = flatRest((func, indexes) =>
			createWrap(
				func,
				WRAP_REARG_FLAG,
				undefined,
		 ...
		 * // => false
		 */
		var isArguments = baseIsArguments((() => arguments)())
			? baseIsArguments
			: (value) =>
					isObjectLike(value) &&
		 ...
		 * // => { 'b': '2' }
		 */
		var omit = flatRest((object, paths) => {
			var result = {};
			if (object == null) {
				return result;
			}
		 ...
		 * // => { 'a': 1, 'c': 3 }
		 */
		var pick = flatRest((object, paths) =>
		...
		function pickBy(object, predicate) {
			...
			return basePickBy(object, props, (value, path) =>
		 ...
		 */
		var method = baseRest(
			(path, args) => (object) => baseInvoke(object, path, args),
		);
		 ...
		 * // => [2, 0]
		 */
		var methodOf = baseRest(
			(object, args) => (path) => baseInvoke(object, path, args),
		);
		...
		arrayEach(
			...
			(methodName) => {
				...
				lodash.prototype[methodName] = function () {
					...
					return this[chainName]((value) =>
						func.apply(isArray(value) ? value : [], args),
					);
				};
			},
		);
