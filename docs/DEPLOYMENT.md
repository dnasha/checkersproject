...
Site: [Astra Checkers](https://dnasha.github.io/checkersproject/)
...
Pages uses **GitHub Actions** as its build source. The existing `github-pages` environment permits the exact `astra-v2.0.0` tag and retains its original `main` branch rule. The workflow grants only the repository-read, artifact-read, Pages-write and identity-token permissions it needs.

## Publish an update

1. Commit and push changes on `astra-ver`, then wait for **Check Astra** to pass.
2. In **Settings → Environments → github-pages → Deployment branches and tags**, add a **tag** rule for the new release name, for example `astra-v2.0.1`.
...
An existing installed client may still show its cached Astra edition. In that browser, use Developer Tools → Application → Service Workers → **Unregister**, then reload. Unregistering the worker preserves the separate local game database; clearing all site data would remove saved games.
...
To return to Astra afterward, select **GitHub Actions** again and rerun the successful Pages release workflow. Do not move an existing release tag to a different commit.
